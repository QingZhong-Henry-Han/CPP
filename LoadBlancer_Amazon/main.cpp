...
public:
  ...
  //11
  bool findIfArrCanDividedIntoThreeEuqal(int arr[], int N)
  {
    for( int i = 1; i < N-2; i++)
      {
	for( int j = i+2; j < N-1; j++)
	  {
	    int sum1 = accumulate(arr, arr+i,0);
	    int sum2 = accumulate(arr+i+1, arr+j,0);
	    int sum3 = accumulate(arr+j+1, arr+N,0);
  ...
  void TestQ11()
  {
    ...
    int N = 9;
    if(findIfArrCanDividedIntoThreeEuqal(arr, N))
      {
	cout << "true\n";
      }
    else
